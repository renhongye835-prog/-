...
/**
 * Enable/disable tolerance mode selector depending on palette mapping.
 * Safe even if toleranceModeSel or palette select is missing.
 */
function syncToleranceUI(){
  const palEl = document.getElementById("palette");
  const palKey = palEl ? (palEl.value || "") : "";
...
toleranceModeSel = el("toleranceMode");
paletteSel && paletteSel.addEventListener("change", syncToleranceUI);
syncToleranceUI();
